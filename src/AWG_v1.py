...
log("#some common waveforms. combine with sum,mult,phasemod")
def sine(x,pars):
    return sin(x*2*pi)
...
log("#pulse: step through rise time,fall time")
...
log("#pulse: step from saw to triangle to saw")
wave1.pars=[0.0,0.0,1.0]
for x in (0.00,0.17,0.33,0.50,0.67,0.83,1.00):
    wave1.pars=[x,0,1-x]
    setupwave(wavbuf[ibuf],freq,wave1); ibuf=(ibuf+1)%2

log("#gaussian: step through widths")
wave1.func=gaussian
for width in (0.15,0.1,0.05,0.02,0.01):
    wave1.pars=[width]
    setupwave(wavbuf[ibuf],freq,wave1); ibuf=(ibuf+1)%2

log("#sinc: step through widths")
wave1.func=sinc
wave1.amplitude=0.5
wave1.offset=0.2
...
log("#exponential: step through widths")
...
log("# reverse exponential: step through widths")
wave1.replicate=-1
for width in (0.20,0.10,0.05,0.02):
    wave1.pars=[width]
    setupwave(wavbuf[ibuf],freq,wave1); ibuf=(ibuf+1)%2

log("# noise: step through quality")
wave1.replicate=1
wave1.func=noise
wave1.amplitude=0.2
wave1.offset=0.5
...
log("# summation: build up a square wave from sines")
wi={}
for i in range(6):
    wi[i]=wave()
...
log("#summation: exponential + gaussians")
wave1=wave()
wave1.amplitude=0.8
wave1.offset=0.0
wave1.phase=0.0
...
log("# modulate: sine with sine - small beta")
wave1=wave()
wave1.amplitude=0.5
wave1.offset=0.5
wave1.phase=0.0
...
log("# modulate: sine with sine - large beta")
wave1=wave()
wave1.amplitude=0.5
wave1.offset=0.5
wave1.phase=0.0
